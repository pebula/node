...
describe('goosetyped', () => {
  ...
  describe('mixins', () => {
    ...
    it('should apply query helper mixins', async () => {
      class QHelper {
        nameWithT<T extends Document & { name: string }>(this: DocumentQuery<T[], T, QHelper>): DocumentQuery<T[], T, QHelper> & QHelper {
          // We have to explicitly type-cast since `this.find` knows about T, it looses the internal implementation.
          // On direction use (Model.find(...).find(...)) it will be ok since find will have the actual type!
          return this.find({ name: /^t.+/ } as unknown as FilterQuery<T>);
        }

        ownerEndWithNum<T extends Document & { owner: string }>(this: DocumentQuery<T[], T, QHelper>): DocumentQuery<T[], T, QHelper> & QHelper {
          return this.find({ owner: /\d$/ } as unknown as FilterQuery<T>);
        }
      }
