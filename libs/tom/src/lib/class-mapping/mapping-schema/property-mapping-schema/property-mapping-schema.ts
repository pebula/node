...
import { getSchema, TypeSystem, TomPropertySchema, TomPropertySchemaConfig } from '../../../schema';
...
export class PropertyMappingSchema<S, T, TKey extends keyof T = keyof T, SKey extends keyof S = keyof S> implements RawPropertyMappingSchema<S, T, TKey, SKey> {
  ...
  @LazyInit(function(this: PropertyMappingSchema<any, any>) {
    if (this.sourceType && this.hasSourcePath && this.sourcePath.length === 1) {
      const propSchema = getSchema(this.sourceType)?.getProperty(this.sourcePath[0]);
      if (propSchema) {
        return propSchema;
      }
    }
    ...
    if (this.sourceKeyType) {
      ...
      if (type) {
        const schemaConfig = new TomPropertySchemaConfig(this.targetKey as string);
        ...
        return new TomPropertySchema(this.sourceKeyType.prototype, schemaConfig);
      }
    }
  })
  sourcePropMeta: TomPropertySchema<S, SKey>;

  @LazyInit(function(this: PropertyMappingSchema<any, any>) {
    return getSchema(this.target).getProperty(this.targetKey);
  })
  targetPropMeta: TomPropertySchema<T, TKey>;
  ...
  createChild(targetPropMeta: TomPropertySchema, sourcePropMeta?: TomPropertySchema) {
  ...
  private _resolveSourceTyped() {
    ...
    switch (this.valueAccessorType) {
      ...
      case 'fixedValue':
        if (isPrimitive(this.value) && this.value?.constructor) {
  ...
  private createSubType(subTypeProp: TomPropertySchema<T, TKey>) {
    const subType = new PropertyMappingSchema({}, this.targetKey, this.sourceType, this.target);
    subType.copyByRef = this.copyByRef;
