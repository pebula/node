...
function prepareUnionTypeList(context: SerializerContext,
                              unionType: TomPropertySchema,
                              isSerialize: boolean): { subType: TomPropertySchema, detector: TypeCompilerHandler}[] {
  const { sorted, classLikeProperties } = determineUnionListResolveOrder(unionType);
  const result: Array<{ subType: TomPropertySchema, detector: TypeCompilerHandler}> = [];
...
function createSubPropertySerializer(unionType: PropertySerializerSchema<any>, propSchema: TomPropertySchema): PropertySerializerSchema<any> {
