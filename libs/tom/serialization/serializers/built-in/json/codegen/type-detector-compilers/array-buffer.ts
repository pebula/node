...
export function arrayBufferSerialize(ctx: CompilerCodeBlockContext<C.ConditionalBlock<any>>, prop: CompilerPropertyContext<JsonSerializer>) {
  const block = ctx.currentBlock.setCondition(`${ctx.sourceAccessor} instanceof ArrayBuffer`);
  return ctx.clone(block);
}

export function arrayBufferDeserialize(ctx: CompilerCodeBlockContext<C.ConditionalBlock<any>>, prop: CompilerPropertyContext<JsonSerializer>) {
