...
export abstract class Serializer<TOptions extends SerializerFactoryOptions = SerializerFactoryOptions> {
  ...
  protected setTypeCompiler(type: TypeSystem.TypeDef): TypeCompiler {
    const compiler = new TypeCompiler();
    this.context.typeCompilers.set(type, compiler);
    return compiler;
  }

  protected setTypeDetectorCompiler(type: TypeSystem.TypeDef): TypeCompiler<C.ConditionalBlock<any>> {
  ...
  protected setRuntimeConverter<T = any>(type: TypeSystem.TypeDef): RuntimeConverter<T> {
  ...
  protected setRuntimeTypeDetector<T = any>(type: TypeSystem.TypeDef): RuntimeConverter<T> {
    const rtConverter = new RuntimeConverter();
