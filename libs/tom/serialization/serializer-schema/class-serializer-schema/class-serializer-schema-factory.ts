...
import { Schema, Nominal } from '@pebula/tom';
...
export class ClassSerializerSchemaFactory<S extends Serializer, T, TData = any, TOriginal extends T = T, TOp extends SerializerOp = SerializerOp> {
  ...
  seal(operation?: TOp): any | void {
    ...
    const schema = Schema.getSchema(this.target);
    ...
    if (properties) {
      ...
      for (const [key, prop] of Object.entries(properties) as Array<[keyof T, Schema.TomPropertySchema]>) {
